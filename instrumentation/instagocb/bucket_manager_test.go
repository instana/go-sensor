...
func TestBucketManager(t *testing.T) {
	...
	// Get
	bsRes, err := bucketMgr.GetBucket(bucketName, &gocb.GetBucketOptions{})
	a.NoError(err)
	...
	// Flush
	err = bucketMgr.FlushBucket(bucketName, &gocb.FlushBucketOptions{})
	a.NoError(err)
	span = getLatestSpan(recorder)
	a.Equal(0, span.Ec)
	a.EqualValues(instana.ExitSpanKind, span.Kind)
	...
	// Drop
	err = bucketMgr.DropBucket(bucketName, &gocb.DropBucketOptions{})
	a.NoError(err)
	span = getLatestSpan(recorder)
	a.Equal(0, span.Ec)
	a.EqualValues(instana.ExitSpanKind, span.Kind)
