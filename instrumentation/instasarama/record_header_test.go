...
func TestPackUnpackTraceContextHeader(t *testing.T) {
	examples := map[string]struct {
		...
		"with short 128-bit trace id, no span id": {
			TraceID: "deadbeefdeadbeefdeadbeef",
			Expected: [24]byte{
				// trace id
		...
		"with 128-bit trace id, no span id": {
		...
		"with 128-bit trace id and 64-bit span id": {
			TraceID: "abcdef12abcdef12abcdef12",
			SpanID:  "deafbeef",
			Expected: [24]byte{
				// trace id
				...
				// span id
				0x00, 0x00, 0x00, 0x00, 0xde, 0xaf, 0xbe, 0xef,
			},
		},
	}
