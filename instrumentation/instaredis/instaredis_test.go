...
func parseIncomingCommand(incomingCmd []byte) ([]byte, error) {
	...
	// This one is tricky. For every Redis command you wanna test, you need to tell go-redis that the command is available.
...
func (c *MockConn) Read(b []byte) (n int, err error) {
	...
	// By just returning the command back, we can successfully test single Redis (NewClient)
	if c.clientType == Single {
		...
		parsedCmd, err := parseIncomingCommand(c.connData)
	...
	for _, rType := range rTypes {
		for name, example := range examples {
			t.Run(fmt.Sprintf("%s - %s", redisTypeMap[rType], name), func(t *testing.T) {
				...
				assert.Equal(t, data.Tags.Error, example.Expected.Error)
				assert.Equal(t, data.Tags.Command, example.Expected.Command)

				if len(example.Expected.Subcommands) > 0 {
					assert.Equal(t, data.Tags.Subcommands, example.Expected.Subcommands)
				}
			})
		}
	}
